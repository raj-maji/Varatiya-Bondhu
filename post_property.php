...
if(isset($_POST['post'])){
   ...
   $video_01_ext = pathinfo($video_01, PATHINFO_EXTENSION);
   $allowed_video_ext = ['mp4', 'webm', 'ogg']; // restrict to safe formats
   $rename_video_01 = create_unique_id().'.'.$video_01_ext;
   $video_01_tmp_name = $_FILES['video_01']['tmp_name'];
   $video_01_size = $_FILES['video_01']['size'];
   $video_01_folder = 'uploaded_files/'.$rename_video_01;

   if(!empty($video_01)){
      if(!in_array($video_01_ext, $allowed_video_ext)){
         $warning_msg[] = 'unsupported video format!';
      } elseif($video_01_size > 20000000){ // limit: 20MB
         $warning_msg[] = 'video size is too large!';
      } else {
         move_uploaded_file($video_01_tmp_name, $video_01_folder);
      }
   } else {
      $rename_video_01 = '';
   }
   ...
   if($image_01_size > 2000000){
      ...
      $insert_property = $conn->prepare("INSERT INTO `property`(
         id, user_id, property_name, address, price, type, status, furnished, bhk, deposite, bedroom, bathroom, balcony, carpet, age, total_floors, room_floor, 
         lift, security_guard, play_ground, garden, water_supply, power_backup, parking_area, gym, shopping_mall, hospital, school, market_area, 
         image_01, image_02, image_03, image_04, image_05, video_01, description
      ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
      
      $insert_property->execute([
         $id, $user_id, $property_name, $address, $price, $type, $status, $furnished, $bhk, $deposite, $bedroom, $bathroom, $balcony, $carpet, $age,
         $total_floors, $room_floor, $lift, $security_guard, $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall,
         $hospital, $school, $market_area, $rename_image_01, $rename_image_02, $rename_image_03, $rename_image_04, $rename_image_05, $rename_video_01, $description
      ]);
...
<section class="property-form">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="box">
         ...
         <input type="text" name="Room_Name" required maxlength="50" placeholder="enter property name" class="input">
      </div>
      <div class="flex">
         <div class="box">
            <p>Room price <span>*</span></p>
      ...
      <div class="box">
         <p>Room description <span>*</span></p>
      ...
      <div class="checkbox">
         ...
         <div class="box">
            ...
            <p><input type="checkbox" name="shopping_mall" value="yes" />nearby shopping_mall</p>
            <p><input type="checkbox" name="hospital" value="yes" />nearby hospital</p>
            <p><input type="checkbox" name="school" value="yes" />nearby school</p>
            <p><input type="checkbox" name="market_area" value="yes" />nearby market</p>
         </div>
      </div>
      <div class="box">
      ...
      <div class="box">
   <p>Property Video (optional, max 20MB)</p>
   <input type="file" name="video_01" class="input" accept="video/mp4, video/webm, video/ogg">
</div>
